class PhysicsRenderer {
  ...
  class Points {
    ...
    createObj(renderer) {
      const detail = window.innerWidth > 768 ? 5 : 4;
      const geometry = new THREE.OctahedronBufferGeometry(300, detail);
      const verticesBase = geometry.attributes.position.array;
      const vertices = [];
      for (var i = 0; i < verticesBase.length; i += 3) {
        vertices[i + 0] = verticesBase[i + 0] + (Math.random() * 2 - 1) * 200;
        vertices[i + 1] = verticesBase[i + 1] + (Math.random() * 2 - 1) * 200;
        vertices[i + 2] = verticesBase[i + 2] + (Math.random() * 2 - 1) * 200;
      }
  ...
  const on = () => {
    ...
    canvas.addEventListener('mousemove', function (event) {
      ...
      // camera.position.set(event.clientX-800, event.clientY-400, 1000);
    ...
    document.onmousemove = function (e) {
      ...
      scene.rotation.x = ((e.clientY - centerY) / centerY) *0.5;
      scene.rotation.y = ((e.clientX - centerX) / centerX) *0.5;
  
    };
  
    document.onwheel = function (e) {
      // e.preventDefault();
      // vectorTouchMove.set(e.touches[0].clientX, e.touches[0].clientY);
      // normalizeVector2(vectorTouchMove);
      // touchMove(true);
      var centerX = window.innerWidth * 0.4;
      var centerY = window.innerHeight * 0.4;
